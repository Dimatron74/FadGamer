<template>
  <header class="fixed top-0 left-0 right-0 z-50 bg-myblack-2 text-mywhite-3 border-b border-myblack-3" style="font-family: 'Roboto', sans-serif; font-weight: bold;">
    <div class="flex p-6 flex-col md:flex-row items-center h-20">
      ...
      <nav class="items-center text-lg justify-between ml-[74px] tracking-wider gap-2 hidden tablet-max:flex desktop-min:gap-10">
        ...
        <RouterLink to="/news" class="hover:text-mywhite-5" @click="scrollToTop">{{ $t('navigation.news') }}</RouterLink>
